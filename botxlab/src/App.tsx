...
import { 
  ...
  Sparkles, Book, Globe, Heart, Scale, CircleDot, GitPullRequest, 
...
const ReadmeSkeleton = () => (
  <div className="space-y-6 max-w-4xl animate-fade-in p-1">
    <Skeleton className="h-10 rounded w-1/4 mb-8" />
    <div className="space-y-3">
      <Skeleton className="h-4 rounded w-full" />
      <Skeleton className="h-4 rounded w-full" />
      <Skeleton className="h-4 rounded w-5/6" />
    </div>
    <Skeleton className="h-48 rounded-xl w-full my-6" />
    <div className="space-y-3">
      <Skeleton className="h-4 rounded w-full" />
...
const ListSkeleton = () => (
  <div className="glass-panel rounded-xl border border-github-border overflow-hidden">
    {[...Array(3)].map((_, i) => (
      <div key={i} className="p-5 border-b border-github-border/50 flex justify-between items-center gap-4">
        <div className="w-full space-y-2">
          <Skeleton className="h-5 rounded w-1/3" />
          <Skeleton className="h-3 rounded w-1/4" />
        </div>
        <Skeleton className="w-16 h-8 rounded shrink-0" />
      </div>
    ))}
  </div>
);
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'readme' | 'releases' | 'commits' | 'collaborators'>('readme');
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        else if (isSidebarOpen && window.innerWidth < 768) setIsSidebarOpen(false);
        else if (searchTerm) setSearchTerm('');
      }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [isSettingsOpen, isSidebarOpen, searchTerm]);
  ...
  const fetchRepos = async (force = false) => {
    ...
    try {
      ...
      const processed = data.sort((a: Repo, b: Repo) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime());
  ...
  useEffect(() => {
    if (view === 'project' && currentRepo) loadTabContent();
  }, [view, activeTab, currentRepo]);

  // TOC Generation
  useEffect(() => {
    if (activeTab === 'readme' && readmeContent && !contentLoading) {
      const timer = setTimeout(() => {
        const headings = document.querySelectorAll('.markdown-body h1, .markdown-body h2, .markdown-body h3');
        const newToc: TocItem[] = Array.from(headings).map(h => ({
          id: h.id,
          text: h.textContent || '',
  ...
  }, [readmeContent, activeTab, contentLoading]);

  const loadTabContent = async () => {
    if (!currentRepo) return;
    setContentLoading(true);
    setContentError(null);
    setReadmeContent(''); setReleases([]); setCommits([]); setCollaborators([]);
    
    try {
      if (activeTab === 'readme') {
        const res = await fetch(`https://api.github.com/repos/${currentRepo.full_name}/readme`, {
           headers: { ...getHeaders(), 'Accept': 'application/vnd.github.v3.raw' }
        });
        if (!res.ok) throw new Error("README not found");
        setReadmeContent(await res.text());
      } else if (activeTab === 'releases') {
        const res = await fetch(`https://api.github.com/repos/${currentRepo.full_name}/releases`, { headers: getHeaders() });
        const data = await res.json();
        setReleases(data);
        if(data.length === 0) throw new Error("No releases found");
      } else if (activeTab === 'commits') {
        const res = await fetch(`https://api.github.com/repos/${currentRepo.full_name}/commits?per_page=20`, { headers: getHeaders() });
        setCommits(await res.json());
      } else if (activeTab === 'collaborators') {
        const res = await fetch(`https://api.github.com/repos/${currentRepo.full_name}/contributors`, { headers: getHeaders() });
        const data = await res.json();
  ...
  const topLang = languages.sort((a,b) => languages.filter(v => v===a).length - languages.filter(v => v===b).length).pop() || 'None';
    ...
    <div className="h-screen flex overflow-hidden font-sans text-sm bg-github-bg text-github-text selection:bg-github-accent selection:text-white">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#010409]/95 border-r border-github-border flex flex-col transform transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} shadow-2xl backdrop-blur-md`}>
        <div className="p-5 flex-shrink-0">
          ...
          <div className="relative group">
            ...
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none flex items-center gap-1">
                <span className="text-[10px] text-github-muted bg-github-border/50 px-1.5 py-0.5 rounded font-mono border border-github-border/50 hidden lg:inline">⌘K</span>
            </div>
          </div>
        </div>
            ...
            {reposLoading ? <SidebarSkeleton /> : (
              <div className="space-y-0.5 pb-4">
                ...
                {filteredRepos.map(repo => (
                  ...
                  >
                    <div className="w-1.5 h-1.5 rounded-full flex-shrink-0 shadow-[0_0_8px_rgba(255,255,255,0.3)] transition-transform group-hover:scale-125" style={{ backgroundColor: stringToColor(repo.language) }}></div>
        ...
        <div className="p-4 border-t border-github-border flex justify-between items-center bg-[#010409]">
           ...
           <button onClick={() => fetchRepos(true)} className="text-github-muted hover:text-white transition-colors p-1.5 hover:bg-github-border rounded-md active:scale-95" title="Refresh"><RefreshCw className={`w-3.5 h-3.5 ${reposLoading ? 'animate-spin' : ''}`} /></button>
        </div>
      </aside>
          ...
          <div className="flex items-center gap-2 md:gap-6 overflow-hidden w-full h-full">
            ...
            <div className="flex flex-col justify-center h-full min-w-0 flex-1 overflow-hidden">
               ...
               {view === 'project' && currentRepo && (
                 <div className="hidden md:flex items-center gap-3 mt-0.5 text-xs text-github-muted overflow-x-auto hide-scrollbar whitespace-nowrap animate-slide-up">
                    {currentRepo.homepage && <a href={currentRepo.homepage} target="_blank" className="flex items-center gap-1.5 px-2 py-0.5 bg-github-accent/10 text-github-accent rounded border border-github-accent/20 hover:bg-github-accent/20 transition-colors"><Globe className="w-3 h-3" /> Website</a>}
                    ...
                    <a href={`${currentRepo.html_url}/issues`} target="_blank" className="flex items-center gap-1.5 px-2 py-0.5 bg-github-card border border-github-border rounded text-github-text hover:border-github-accent transition-colors"><CircleDot className="w-3 h-3 text-github-muted" /> {currentRepo.open_issues_count}</a>
                 </div>
               )}
            </div>
            ...
            {view === 'project' && currentRepo && (
               <div className="flex ml-2 md:ml-4 items-center gap-1 md:gap-3 border-l border-github-border pl-2 md:pl-4 h-8 my-auto shrink-0 animate-fade-in">
                  ...
                  }} className="text-github-muted hover:text-white transition-colors p-1.5 hover:bg-github-border rounded-md group relative active:scale-95">
                    {repoCopied ? <Check className="w-5 h-5 text-green-500 animate-bounce" /> : <Code2 className="w-5 h-5" />}
                  </button>
                  <a href={currentRepo.html_url} target="_blank" className="text-github-muted hover:text-white transition-colors p-1.5 hover:bg-github-border rounded-md active:scale-95"><ExternalLink className="w-5 h-5" /></a>
               </div>
            )}
          </div>
        </header>
             ...
             {view === 'dashboard' && (
               <div className="animate-slide-up">
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 mb-8">
                     {[
                       { label: 'Total Projects', value: repos.length, icon: FolderGit2, color: 'text-blue-400', border: 'hover:border-blue-500/30', bg: 'bg-blue-500/10' },
                       { label: 'Total Stars', value: totalStars, icon: Star, color: 'text-yellow-400', border: 'hover:border-yellow-500/30', bg: 'bg-yellow-500/10' },
                       { label: 'Top Language', value: topLang, icon: Code2, color: 'text-purple-400', border: 'hover:border-purple-500/30', bg: 'bg-purple-500/10' }
                     ].map((stat, i) => (
                       <div key={i} className={`glass-panel rounded-xl p-6 relative overflow-hidden group transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl ${stat.border}`}>
                          <div className={`absolute -right-6 -top-6 w-32 h-32 rounded-full blur-3xl group-hover:bg-opacity-20 transition-all duration-500 ${stat.bg}`}></div>
                          <div className="relative z-10">
                             <div className="flex items-center justify-between mb-2">
                                <p className="text-github-muted text-xs font-bold uppercase tracking-wider">{stat.label}</p>
                                <stat.icon className={`w-4 h-4 opacity-80 ${stat.color}`} />
                             </div>
                             {reposLoading ? <Skeleton className="h-8 w-16 rounded" /> : <div className="text-4xl font-bold text-white tracking-tight animate-fade-in">{stat.value}</div>}
                          </div>
                       </div>
                     ))}
                  </div>
                     ...
                     {reposLoading ? (
                       ...
                       repos.slice(0, 6).map(repo => (
                         <div key={repo.id} onClick={() => handleOpenProject(repo)} className="glass-panel p-6 rounded-xl cursor-pointer hover:border-github-accent/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-blue-500/10 group flex flex-col h-full">
                            ...
                            {/* --- NEW: VISIBLE UPDATE TIME FOR SORTING --- */}
                  ...
                  <div className="flex-1 min-w-0 w-full">
                     {contentLoading ? (
                        <div className="animate-fade-in">
                           {activeTab === 'readme' ? <ReadmeSkeleton /> : <ListSkeleton />}
                        </div>
                     ) : contentError ? (
                              ...
                              <ReactMarkdown
                                ...
                                components={{
                                  img: ({node, ...props}) => {
                                    ...
                                    return <img {...props} src={url} style={{maxWidth: '100%'}} alt={props.alt || ''} />;
                                  },
                                  a: ({node, ...props}) => {
                                    let url = props.href || '';
                              ...
                              {releases.map((release: any) => (
                                <div key={release.id} className="glass-panel p-6 rounded-xl border border-github-border transition-transform hover:scale-[1.005]">
                                   <div className="flex justify-between items-start mb-4">
                                      ...
                                      <a href={release.html_url} target="_blank" className="px-4 py-2 text-xs font-medium bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg transition-colors text-white flex-shrink-0 active:scale-95">View on GitHub</a>
                                   </div>
                              ...
                              {commits.map((item: any, idx) => (
                                <div key={item.sha} className={`p-4 flex gap-4 group hover:bg-white/5 transition-colors ${idx !== commits.length - 1 ? 'border-b border-github-border' : ''}`}>
                                   ...
                                   <div className="flex flex-col items-end justify-center shrink-0">
                                      <a href={item.html_url} target="_blank" className="text-[10px] font-mono text-github-muted hover:text-white bg-white/5 px-2 py-1 rounded border border-white/5 transition-colors hover:border-github-accent/50">{item.sha.substring(0,7)}</a>
                                   </div>
                                </div>
                              ))}
                           </div>
                           ...
                           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 animate-slide-up">
                              {collaborators.map((user: any) => (
                                <div key={user.id} className="glass-panel p-4 rounded-xl border border-github-border flex items-center gap-4 hover:border-github-accent/50 transition-all hover:-translate-y-0.5 hover:shadow-lg group min-w-0">
                                   ...
                                   <div className="flex-1 min-w-0">
                                      <a href={user.html_url} target="_blank" className="text-white font-medium hover:text-github-accent truncate block text-sm transition-colors">{user.login}</a>
                                   ...
                                   <a href={user.html_url} target="_blank" className="p-2 text-github-muted hover:text-white bg-white/5 hover:bg-white/10 rounded-lg transition-colors shrink-0 active:scale-95"><ExternalLink className="w-4 h-4" /></a>
                                </div>
                              ))}
                           </div>
                         )}
